...
```bash
node scripts/import-batch.mjs                       # 扫 data/raw，3 路并行
node scripts/import-batch.mjs --lanes 4 --only lunyu,daodejing
```

单篇失败不拖垮整批，日志落在 `.import-logs/<id>.log`；完成记录带原文指纹，
原文没改就跳过，失败的直接重跑同一条命令即可续上。长篇还会逐章落
`.import-cache/`，中途断掉不必从头再来。两个目录都不进版本库。
...
- **切句不切进引号里**。`「子曰：“…乎？…乎？”」` 从问号处断开会在站上留下不闭合的半句，模型逐句回抄时又会顺手补全，触发原文校验反复打回。
- **原文一字不改**是硬校验。`annotate` 会逐句比对模型返回的 `text` 与投入的原文，不一致就打回。
- **原文不留扩展区汉字**。`𫄨`（絺）`𫐐`（輗）这类扩展区字模型回抄不出来，校验会一轮轮打回；多数字体也缺字，读者那头是个空框。没有通行简体写法的字一律用基本区正体（`骖騑`／`距闉`）。
- **长章分片译注**。骈文与典籍一章能有七八十句，一次交付输出量大到交不完，且校验一打回就要整份重写。`annotate` 按句数封顶分片，大意与赏析另起一轮。
- **译注逐章落断点**（`.import-cache/`）。八十一章的典籍跑到一半断掉，不该把前面几十章重烧一遍。产物 JSON 要等导读写完才算合法，中途只能落在这个一次性缓存里。
